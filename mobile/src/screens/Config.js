...
export default function ConfigScreen({ navigation }) {
  const {
    ...
    tema,
    handleChangeTema,
    twitchUser, setTwitchUser, handleSaveTwitch,
  ...
  const handleShare = async () => {
    try {
      const result = await Share.share({
        message:
          "Gostei deste app de pads da IF, da uma olhada ae: https://play.google.com/store/apps/details?id=com.ifpads.streamers",
      });
    } catch (error) {
      Alert.alert(
  ...
  return (
    <SafeAreaView style={{ ...styles.container, backgroundColor: theme[tema].background }}>
      <DialogComponent
        isVisible={isActiveDialog}
        title="Vincule sua Twitch"
        description="Digite seu @ na twitch"
        label="Nome de usuário"
        onChangeText={(text) => setTwitchUser(text)}
        value={twitchUser}
      ...
      <ScrollView style={{ width: "100%", paddingHorizontal: 16 }}>
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <FontAwesome color={"#FFF"} name="twitch" size={22} />
            <Text style={{ marginLeft: 10, color: "#FFF" }}>Habilitar controle Twitch</Text>
          </View>
        </TouchableOpacity>
